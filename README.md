...
На macOS с Docker Desktop может понадобиться:

```bash
DOCKER_HOST=unix:///Users/$USER/.docker/run/docker.sock PASSWORD_SERVICE_IT=1 sbt passwordService/testOnly ru.gpn.password.PostgresPasswordRepositorySpec
```

## Что Проверено

- `sbt compile` проходит.
- `sbt test` проходит: `31` быстрый тест, `0` failed.
- Локальный HTTP smoke для `url-downloader-service` проходил на `:19082`: `/health`, `/openapi.json`, `POST /downloads`, `GET /downloads/{jobId}`.
- Testcontainers в текущей Codex/sandbox-сессии не смог увидеть Docker environment, хотя `docker ps` снаружи отвечал. Прямой `docker compose up -d postgres` начал pull образа PostgreSQL, но загрузка была слишком медленной и была остановлена.

## Замечания

- Password CSV import создает новые записи по `name/password/comment`; `id/created/deleted` из экспортированного CSV намеренно не восстанавливаются.
- URL downloader хранит статусы job в памяти, поэтому они теряются при рестарте сервиса.
- Swagger UI HTML использует CDN assets, но `/openapi.json` полностью локальный.
